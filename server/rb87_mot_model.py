...
wavelength_fluctuation = 1e-8  # процент флуктуации длины волны
power_fluctuation = 0.05  # % флуктуации мощности
magnetic_fluctuation = 0.03  # % флуктуации магнитного поля
...
repumper_effect = 1  # коэффициент влияния репампера на переходы

# Параметры симуляции
atoms_quantity = 600  # Число атомов — в реальности 500 000
nsim = 1  # Количество симуляций
timesim = 1e-5  # Период симуляции
dtsim = 1e-9  # Шаг симуляции (в секундах)
itera = timesim / dtsim
...
def repumper_effect_on_levels(levels, velocities, I_repumper, temperature, delta_wavelength=wavelength_fluctuation):
    ...
    leak_prob = 1e-5  # Маленькая вероятность утечки на темновый уровень (параметр можно подстраивать)
